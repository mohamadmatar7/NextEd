...
<x-app-layout>
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl w-full lg:py-2 h-full">
        <div class="py-6 px-4 sm:px-10 bg-white dark:bg-gray-800">
            ...
            <!-- Header Section -->
            <div
                class="bg-gradient-to-r from-blue-500 to-blue-700 h-48 flex items-center justify-center rounded-ss-lg rounded-se-lg">
                <div class="relative">
                    @if ($user->avatar)
                    ...
                    @else
                    <div
                        class="w-32 h-32 rounded-full bg-gray-300 flex items-center justify-center text-white text-4xl">
            ...
            <!-- Main Content -->
            <div class="p-8">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">{{ $user->first_name }} {{
                    $user->last_name }}</h1>
                <p class="text-gray-600 dark:text-gray-400 mb-4">{{ $user->role == 0 ? 'Student' : ($user->role == 1 ?
                    'Teacher' : ($user->role == 2 ? 'Instructor' : ($user->role == 3 ? 'Principal' : 'Admin'))) }}</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Contact -->
                    <div>
                        <h2 class="text-gray-800 dark:text-gray-200 font-semibold mb-2 text-xl">Contact</h2>
                        <p class="text-gray-600 dark:text-gray-400 break-all"><strong>Email:</strong> {{ $user->email }}
                        </p>
                        <p class="text-gray-600 dark:text-gray-400"><strong>Gender:</strong> {{ ucfirst($user->gender)
                            }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 dark:text-gray-400"><strong>Joined:</strong> {{
                            $user->created_at->format('F j, Y') }}</p>
                        @if ($user->email_verified_at)
                        <p class="text-gray-600 dark:text-gray-400"><strong>Email Verified:</strong> {{
                            $user->email_verified_at->format('F j, Y') }}</p>
                        @else
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 dark:text-gray-400"><strong>Programs:</strong> {{
                            $user->courses->pluck('program')->unique()->implode('name', ', ') }}</p>
                    </div>

                    @auth
                    @if (Auth::user()->id == $user->id || Auth::user()->can('is-administrator'))
                    <!-- Course -->
                    <div class="mb-4">
                        ...
                        <div class="mt-4 flex flex-col justify-end">
                            <strong
                                class="w-fit inline-flex md:text-sm lg:text-base text-white font-bold p-2 sm:py-2 sm:px-4 rounded bg-gradient-to-r from-blue-500 to-blue-700 dark:from-gray-700 dark:to-gray-900 hover:from-blue-700 hover:to-blue-900 cursor-pointer"
                                id="add-btn">
                            ...
                            <x-form-add-to :searchFor="__('template.courses')" type="courses"
                                addRoute="{{ route('users.storeCoursesToUser', ['user' => $user->id]) }}" single="user"
                                singleId="{{ $user->id }}" />
                        </div>
                        @endcan
                    </div>
