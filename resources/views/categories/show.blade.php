        ...
        @slot('tableRows')
            @foreach($category->programs as $program)
                <tr class="search-row">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 program-courses-count">{{ $program->courses->count() }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 program-lessons-count">{{ $program->courses->map(function($course) { return $course->lessons->count(); })->sum() }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 program-users-count">{{ $program->courses->map(function($course) { return $course->users->where('role', 0)->count(); })->sum() }}</td>
